...
say "Launcher"
mkdir -p "$BIN_DIR"
cat > "$BIN_DIR/xtouchmini" <<LAUNCHER
#!/bin/sh
# Generated by install.sh -- the checkout is the installation.
PYTHONPATH="$HERE\${PYTHONPATH:+:\$PYTHONPATH}" exec /usr/bin/python3 -m xtouchmini "\$@"
LAUNCHER
chmod +x "$BIN_DIR/xtouchmini"
